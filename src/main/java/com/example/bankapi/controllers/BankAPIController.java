...
public class BankAPIController {
    ...
    public ResponseEntity<List<CardRespDTO>> indexCard(@PathVariable long accountId) throws EmptyAccountException {
        List<CardRespDTO> cards = clientService.getClientCards(accountId);
        return ResponseEntity.ok().body(cards);
    }

    @PostMapping(value = "/{accountId}/card")
    public void newCard(@PathVariable long accountId,
                        @RequestBody CardReqDTO cardReqDTO) throws ThereIsNoSuchAccountException, NotEqualAccountIdException {
        clientService.addNewCardByAccountId(accountId, cardReqDTO);
    }

    @GetMapping(value = "/{accountId}/balance")
    public ResponseEntity<AccRespDTO> showBalance(@PathVariable long accountId) throws ThereIsNoSuchAccountException {
        AccRespDTO dto = clientService.getClientAccountBalance(accountId);
        return ResponseEntity.ok().body(dto);
    }

    @PutMapping(value = "/{accountId}/balance")
    public void depositMoney(@PathVariable long accountId,
                             @RequestBody AccReqDTO accReqDTO) throws ThereIsNoSuchAccountException {
